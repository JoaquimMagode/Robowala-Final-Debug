import { PrismaClient } from "@prisma/client"
import bcrypt from "bcryptjs"

const prisma = new PrismaClient()

async function main() {
  console.log("🌱 Starting database seed...")

  // Create admin user
  const hashedPassword = await bcrypt.hash("admin123", 12)
  const admin = await prisma.user.upsert({
    where: { email: "admin@robowala.com" },
    update: {},
    create: {
      email: "admin@robowala.com",
      name: "Admin User",
      password: hashedPassword,
      role: "ADMIN",
    },
  })
  console.log("✅ Created admin user:", admin.email)

  // Create test user
  const testHashedPassword = await bcrypt.hash("test123", 12)
  const testUser = await prisma.user.upsert({
    where: { email: "test@example.com" },
    update: {},
    create: {
      email: "test@example.com",
      name: "Test User",
      password: testHashedPassword,
      role: "USER",
    },
  })
  console.log("✅ Created test user:", testUser.email)

  // Comprehensive product list matching robu.in
  const products = [
    // Raspberry Pi Products
    {
      name: "Raspberry Pi 5 8GB",
      slug: "raspberry-pi-5-8gb",
      price: 7499,
      originalPrice: 8499,
      rating: 4.9,
      reviews: 156,
      image: "/raspberry-pi-5-single-board-computer.jpg",
      category: "Raspberry Pi",
      categorySlug: "raspberry-pi",
      subcategory: "Raspberry Pi 5",
      subcategorySlug: "raspberry-pi-5",
      brand: "Raspberry Pi Foundation",
      badge: "New",
      inStock: true,
      description: "The most powerful Raspberry Pi yet with 8GB RAM, quad-core Cortex-A76 processor, and enhanced connectivity.",
      specifications: JSON.stringify({
        CPU: "Quad-core Cortex-A76 @ 2.4GHz",
        RAM: "8GB LPDDR4X",
        Storage: "microSD card slot",
        USB: "2x USB 3.0, 2x USB 2.0",
        Display: "2x micro HDMI (4K 60fps)",
        GPIO: "40-pin header",
        Connectivity: "Gigabit Ethernet, WiFi 6, Bluetooth 5.2",
      }),
    },
    {
      name: "Raspberry Pi 4 Model B 4GB",
      slug: "raspberry-pi-4-4gb",
      price: 4999,
      originalPrice: 5999,
      rating: 4.8,
      reviews: 523,
      image: "/raspberry-pi-4-model-b.jpg",
      category: "Raspberry Pi",
      categorySlug: "raspberry-pi",
      subcategory: "Raspberry Pi 4",
      subcategorySlug: "raspberry-pi-4",
      brand: "Raspberry Pi Foundation",
      badge: "Bestseller",
      inStock: true,
      description: "Popular Raspberry Pi 4 with 4GB RAM, perfect for most projects and applications.",
      specifications: JSON.stringify({
        CPU: "Quad-core Cortex-A72 @ 1.5GHz",
        RAM: "4GB LPDDR4",
        USB: "2x USB 3.0, 2x USB 2.0",
        Display: "2x micro HDMI (4K 60fps)",
        GPIO: "40-pin header",
      }),
    },
    {
      name: "Raspberry Pi Pico W",
      slug: "raspberry-pi-pico-w",
      price: 549,
      originalPrice: 699,
      rating: 4.7,
      reviews: 234,
      image: "/raspberry-pi-pico-w.jpg",
      category: "Raspberry Pi",
      categorySlug: "raspberry-pi",
      subcategory: "Raspberry Pi Pico",
      subcategorySlug: "raspberry-pi-pico",
      brand: "Raspberry Pi Foundation",
      badge: "Hot",
      inStock: true,
      description: "Microcontroller board with WiFi, based on RP2040 chip. Perfect for IoT projects.",
      specifications: JSON.stringify({
        CPU: "Dual-core ARM Cortex-M0+ @ 133MHz",
        RAM: "264KB SRAM",
        Flash: "2MB",
        WiFi: "802.11n",
        GPIO: "26 multifunction pins",
      }),
    },

    // Arduino Products
    {
      name: "Arduino UNO R4 WiFi",
      slug: "arduino-uno-r4-wifi",
      price: 2499,
      originalPrice: 2999,
      rating: 4.9,
      reviews: 312,
      image: "/arduino-uno-r4-wifi-development-board-blue.jpg",
      category: "Arduino",
      categorySlug: "arduino",
      subcategory: "Classic Family",
      subcategorySlug: "arduino-classic",
      brand: "Arduino",
      badge: "New",
      inStock: true,
      description: "Latest Arduino UNO with built-in WiFi, enhanced processing power, and LED matrix.",
      specifications: JSON.stringify({
        CPU: "Renesas RA4M1 (Cortex-M4) @ 48MHz",
        Flash: "256KB",
        RAM: "32KB",
        WiFi: "ESP32-S3",
        "Operating Voltage": "5V",
        "Digital I/O": "14 pins",
        "Analog Input": "6 pins",
      }),
    },
    {
      name: "Arduino Nano ESP32",
      slug: "arduino-nano-esp32",
      price: 1799,
      originalPrice: 2199,
      rating: 4.8,
      reviews: 189,
      image: "/arduino-nano-esp32.jpg",
      category: "Arduino",
      categorySlug: "arduino",
      subcategory: "Nano Family",
      subcategorySlug: "arduino-nano",
      brand: "Arduino",
      badge: "Popular",
      inStock: true,
      description: "Compact Arduino with ESP32-S3 for WiFi and Bluetooth connectivity.",
      specifications: JSON.stringify({
        CPU: "ESP32-S3 dual-core @ 240MHz",
        Flash: "8MB",
        RAM: "512KB SRAM",
        WiFi: "802.11 b/g/n",
        Bluetooth: "BLE 5.0",
      }),
    },

    // ESP32/ESP8266 Boards
    {
      name: "ESP32-WROOM-32 DevKit",
      slug: "esp32-wroom-32-devkit",
      price: 499,
      originalPrice: 699,
      rating: 4.8,
      reviews: 678,
      image: "/esp32-development-board-wifi-bluetooth-microcontro.jpg",
      category: "Microcontrollers",
      categorySlug: "microcontrollers",
      subcategory: "ESP32 Boards",
      subcategorySlug: "esp32",
      brand: "Espressif",
      badge: "Bestseller",
      inStock: true,
      description: "Powerful WiFi & Bluetooth enabled microcontroller for IoT projects.",
      specifications: JSON.stringify({
        CPU: "Dual-core Xtensa @ 240MHz",
        Flash: "4MB",
        RAM: "520KB SRAM",
        WiFi: "802.11 b/g/n",
        Bluetooth: "BLE 4.2",
        GPIO: "34 pins",
      }),
    },
    {
      name: "NodeMCU ESP8266",
      slug: "nodemcu-esp8266",
      price: 299,
      originalPrice: 399,
      rating: 4.7,
      reviews: 892,
      image: "/nodemcu-esp8266-development-board-blue.jpg",
      category: "Microcontrollers",
      categorySlug: "microcontrollers",
      subcategory: "ESP8266 Boards",
      subcategorySlug: "esp8266",
      brand: "Espressif",
      badge: "Value",
      inStock: true,
      description: "WiFi enabled development board based on ESP8266, perfect for beginners.",
      specifications: JSON.stringify({
        CPU: "Tensilica L106 @ 80MHz",
        Flash: "4MB",
        RAM: "128KB",
        WiFi: "802.11 b/g/n",
        GPIO: "17 pins",
      }),
    },

    // Drone Parts
    {
      name: "F450 Quadcopter Frame Kit",
      slug: "f450-quadcopter-frame",
      price: 1299,
      originalPrice: 1699,
      rating: 4.6,
      reviews: 145,
      image: "/f450-quadcopter-frame.jpg",
      category: "Drone Parts",
      categorySlug: "drone-parts",
      subcategory: "Frames",
      subcategorySlug: "drone-frames",
      brand: "DJI",
      badge: null,
      inStock: true,
      description: "Durable quadcopter frame kit with 450mm wheelbase, perfect for DIY drones.",
      specifications: JSON.stringify({
        Wheelbase: "450mm",
        Material: "Glass Fiber + Nylon",
        Weight: "282g",
        "Motor Mount": "16mm x 19mm",
        "Recommended Motors": "2212 920KV",
      }),
    },
    {
      name: "2212 920KV Brushless Motor",
      slug: "2212-920kv-motor",
      price: 649,
      originalPrice: 849,
      rating: 4.7,
      reviews: 234,
      image: "/2212-brushless-motor.jpg",
      category: "Drone Parts",
      categorySlug: "drone-parts",
      subcategory: "Drone Motors",
      subcategorySlug: "drone-motors",
      brand: "Emax",
      badge: "Popular",
      inStock: true,
      description: "High-quality brushless motor for quadcopters and multirotors.",
      specifications: JSON.stringify({
        KV: "920",
        "Max Current": "15A",
        Voltage: "11.1V (3S)",
        Weight: "55g",
        "Shaft Diameter": "3.17mm",
      }),
    },
    {
      name: "30A ESC (Electronic Speed Controller)",
      slug: "30a-esc",
      price: 449,
      originalPrice: 599,
      rating: 4.5,
      reviews: 178,
      image: "/30a-esc.jpg",
      category: "Drone Parts",
      categorySlug: "drone-parts",
      subcategory: "ESCs",
      subcategorySlug: "esc",
      brand: "Hobbywing",
      badge: null,
      inStock: true,
      description: "30A brushless ESC with BEC for drone motors.",
      specifications: JSON.stringify({
        "Continuous Current": "30A",
        "Burst Current": "40A",
        BEC: "5V 2A",
        "Input Voltage": "2-4S LiPo",
        Weight: "25g",
      }),
    },

    // Sensors
    {
      name: "DHT22 Temperature & Humidity Sensor",
      slug: "dht22-sensor",
      price: 249,
      originalPrice: 349,
      rating: 4.6,
      reviews: 567,
      image: "/dht22-temperature-humidity-sensor-white.jpg",
      category: "Sensors",
      categorySlug: "sensors",
      subcategory: "Temperature Sensors",
      subcategorySlug: "temperature-sensors",
      brand: "Aosong",
      badge: "Bestseller",
      inStock: true,
      description: "High precision digital temperature and humidity sensor.",
      specifications: JSON.stringify({
        "Temperature Range": "-40°C to 80°C",
        "Temperature Accuracy": "±0.5°C",
        "Humidity Range": "0-100% RH",
        "Humidity Accuracy": "±2% RH",
        Interface: "Single-wire digital",
      }),
    },
    {
      name: "HC-SR04 Ultrasonic Distance Sensor",
      slug: "hc-sr04-ultrasonic",
      price: 79,
      originalPrice: 129,
      rating: 4.7,
      reviews: 892,
      image: "/hc-sr04-ultrasonic-sensor-module-blue.jpg",
      category: "Sensors",
      categorySlug: "sensors",
      subcategory: "Proximity Sensors",
      subcategorySlug: "proximity-sensors",
      brand: null,
      badge: "Value",
      inStock: true,
      description: "Ultrasonic distance measuring sensor module, 2cm to 400cm range.",
      specifications: JSON.stringify({
        Range: "2cm - 400cm",
        Accuracy: "3mm",
        "Measuring Angle": "15°",
        "Trigger Input": "10μs TTL pulse",
        "Operating Voltage": "5V DC",
      }),
    },
    {
      name: "37-in-1 Sensor Module Kit",
      slug: "37-in-1-sensor-kit",
      price: 1299,
      originalPrice: 1799,
      rating: 4.8,
      reviews: 445,
      image: "/arduino-sensor-kit-37-pieces-various-sensors-modul.jpg",
      category: "Sensors",
      categorySlug: "sensors",
      subcategory: "Sensor Kits",
      subcategorySlug: "sensor-kits",
      brand: null,
      badge: "Kit",
      inStock: true,
      description: "Complete sensor kit with 37 different modules for Arduino and Raspberry Pi.",
      specifications: JSON.stringify({
        "Total Sensors": "37",
        Compatible: "Arduino, Raspberry Pi, ESP32",
        Includes: "Temperature, Humidity, Light, Sound, Motion, Gas, etc.",
        Case: "Plastic storage box included",
      }),
    },

    // Motors & Drivers
    {
      name: "MG996R High Torque Servo Motor",
      slug: "mg996r-servo",
      price: 349,
      originalPrice: 449,
      rating: 4.6,
      reviews: 523,
      image: "/mg996r-servo-motor-robotics-blue.jpg",
      category: "Motors & Drivers",
      categorySlug: "motors",
      subcategory: "Servo Motors",
      subcategorySlug: "servo-motors",
      brand: "TowerPro",
      badge: "Popular",
      inStock: true,
      description: "High-torque metal gear servo motor for robotics and RC applications.",
      specifications: JSON.stringify({
        Torque: "13kg/cm @ 6V",
        Speed: "0.13s/60° @ 6V",
        "Operating Voltage": "4.8V - 7.2V",
        Weight: "55g",
        Gear: "Metal",
      }),
    },
    {
      name: "L298N Dual H-Bridge Motor Driver",
      slug: "l298n-motor-driver",
      price: 179,
      originalPrice: 249,
      rating: 4.5,
      reviews: 678,
      image: "/l298n-motor-driver-module-red-pcb.jpg",
      category: "Motors & Drivers",
      categorySlug: "motors",
      subcategory: "Motor Drivers",
      subcategorySlug: "motor-drivers",
      brand: "STMicroelectronics",
      badge: "Bestseller",
      inStock: true,
      description: "Dual H-Bridge motor driver module for controlling DC motors and stepper motors.",
      specifications: JSON.stringify({
        Channels: "2",
        "Max Current": "2A per channel",
        "Peak Current": "3A per channel",
        "Input Voltage": "5V - 35V",
        "Logic Voltage": "5V",
      }),
    },
    {
      name: "NEMA 17 Stepper Motor",
      slug: "nema-17-stepper",
      price: 599,
      originalPrice: 799,
      rating: 4.7,
      reviews: 234,
      image: "/nema-17-stepper-motor.jpg",
      category: "Motors & Drivers",
      categorySlug: "motors",
      subcategory: "Stepper Motors",
      subcategorySlug: "stepper-motors",
      brand: null,
      badge: null,
      inStock: true,
      description: "High precision NEMA 17 stepper motor for 3D printers and CNC machines.",
      specifications: JSON.stringify({
        "Step Angle": "1.8°",
        "Holding Torque": "0.4Nm",
        Voltage: "12V",
        Current: "1.5A",
        "Motor Length": "40mm",
      }),
    },

    // Displays
    {
      name: '0.96" OLED Display I2C 128x64',
      slug: "oled-096-i2c",
      price: 199,
      originalPrice: 299,
      rating: 4.8,
      reviews: 789,
      image: "/oled-display-0-96-inch-i2c-blue-text.jpg",
      category: "Displays",
      categorySlug: "displays",
      subcategory: "OLED Displays",
      subcategorySlug: "oled-displays",
      brand: null,
      badge: "Bestseller",
      inStock: true,
      description: "Compact OLED display module with I2C interface, blue color.",
      specifications: JSON.stringify({
        Size: '0.96"',
        Resolution: "128x64 pixels",
        Interface: "I2C",
        Color: "Blue",
        "Operating Voltage": "3.3V - 5V",
      }),
    },
    {
      name: '2.8" TFT LCD Touch Display',
      slug: "tft-28-touch",
      price: 699,
      originalPrice: 899,
      rating: 4.6,
      reviews: 234,
      image: "/tft-28-touch-display.jpg",
      category: "Displays",
      categorySlug: "displays",
      subcategory: "TFT Displays",
      subcategorySlug: "tft-displays",
      brand: null,
      badge: null,
      inStock: true,
      description: "2.8 inch TFT LCD display with resistive touch screen.",
      specifications: JSON.stringify({
        Size: '2.8"',
        Resolution: "240x320 pixels",
        Interface: "SPI",
        Touch: "Resistive",
        Controller: "ILI9341",
      }),
    },

    // Wireless & IoT
    {
      name: "NRF24L01+ 2.4GHz Wireless Module",
      slug: "nrf24l01-plus",
      price: 149,
      originalPrice: 199,
      rating: 4.5,
      reviews: 456,
      image: "/nrf24l01-wireless-transceiver-module-green-pcb.jpg",
      category: "Wireless & IoT",
      categorySlug: "wireless",
      subcategory: "RF Modules",
      subcategorySlug: "rf-modules",
      brand: "Nordic",
      badge: "Value",
      inStock: true,
      description: "2.4GHz wireless transceiver module for Arduino and Raspberry Pi.",
      specifications: JSON.stringify({
        Frequency: "2.4GHz ISM band",
        "Data Rate": "250Kbps to 2Mbps",
        Range: "100m (open space)",
        "Operating Voltage": "1.9V - 3.6V",
        Interface: "SPI",
      }),
    },
    {
      name: "HC-05 Bluetooth Module",
      slug: "hc-05-bluetooth",
      price: 299,
      originalPrice: 399,
      rating: 4.6,
      reviews: 567,
      image: "/hc-05-bluetooth-module.jpg",
      category: "Wireless & IoT",
      categorySlug: "wireless",
      subcategory: "Bluetooth Modules",
      subcategorySlug: "bluetooth-modules",
      brand: null,
      badge: "Popular",
      inStock: true,
      description: "Bluetooth 2.0 serial module for wireless communication.",
      specifications: JSON.stringify({
        Bluetooth: "v2.0 + EDR",
        Range: "10m",
        "Operating Voltage": "3.3V - 5V",
        Interface: "UART",
        "Baud Rate": "9600 (default)",
      }),
    },

    // Power Supply
    {
      name: "5V 5A Power Adapter with DC Plug",
      slug: "5v-5a-adapter",
      price: 349,
      originalPrice: 449,
      rating: 4.7,
      reviews: 345,
      image: "/5v-5a-power-adapter.jpg",
      category: "Power Supply",
      categorySlug: "power-supply",
      subcategory: "Power Adapters",
      subcategorySlug: "adapters",
      brand: "Pro-Range",
      badge: null,
      inStock: true,
      description: "High quality 5V 5A power adapter with 5.5x2.5mm DC plug.",
      specifications: JSON.stringify({
        "Output Voltage": "5V DC",
        "Output Current": "5A",
        "Connector Type": "5.5mm x 2.5mm DC plug",
        "Input Voltage": "100-240V AC",
        Protection: "Over-current, Over-voltage, Short-circuit",
      }),
    },
    {
      name: "LM2596 Buck Converter Module",
      slug: "lm2596-buck-converter",
      price: 89,
      originalPrice: 129,
      rating: 4.5,
      reviews: 678,
      image: "/lm2596-buck-converter.jpg",
      category: "Power Supply",
      categorySlug: "power-supply",
      subcategory: "Buck Converters",
      subcategorySlug: "buck-converters",
      brand: "Texas Instruments",
      badge: "Value",
      inStock: true,
      description: "Adjustable step-down voltage regulator module.",
      specifications: JSON.stringify({
        "Input Voltage": "4.5V - 40V",
        "Output Voltage": "1.25V - 37V (adjustable)",
        "Output Current": "3A max",
        Efficiency: "Up to 92%",
        "Switching Frequency": "150KHz",
      }),
    },

    // Learning Kits
    {
      name: "4WD Robot Car Chassis Kit",
      slug: "4wd-robot-car-kit",
      price: 1899,
      originalPrice: 2499,
      rating: 4.7,
      reviews: 234,
      image: "/4wd-robot-car-chassis-kit-arduino-compatible.jpg",
      category: "Learning Kits",
      categorySlug: "kits",
      subcategory: "Robotics Kits",
      subcategorySlug: "robotics-kits",
      brand: null,
      badge: "Kit",
      inStock: true,
      description: "Complete 4WD robot car chassis kit with motors and wheels.",
      specifications: JSON.stringify({
        Motors: "4x TT DC Motors",
        Wheels: "4x 65mm diameter",
        Material: "Acrylic",
        "Battery Holder": "4x AA (not included)",
        Dimensions: "200mm x 150mm",
      }),
    },
    {
      name: "Arduino Starter Kit for Beginners",
      slug: "arduino-starter-kit",
      price: 2499,
      originalPrice: 2999,
      rating: 4.8,
      reviews: 456,
      image: "/arduino-starter-kit.jpg",
      category: "Learning Kits",
      categorySlug: "kits",
      subcategory: "Arduino Kits",
      subcategorySlug: "arduino-kits-learning",
      brand: "Arduino",
      badge: "Bestseller",
      inStock: true,
      description: "Complete Arduino starter kit with UNO board and 30+ components.",
      specifications: JSON.stringify({
        Includes: "Arduino UNO R3, Breadboard, LEDs, Resistors, Sensors, Motors, etc.",
        Components: "30+ items",
        Projects: "15 guided projects",
        Manual: "Included",
      }),
    },

    // Tools
    {
      name: "MASTECH MS8233D Digital Multimeter",
      slug: "mastech-ms8233d",
      price: 899,
      originalPrice: 1199,
      rating: 4.7,
      reviews: 234,
      image: "/mastech-ms8233d-multimeter.jpg",
      category: "Tools & Equipment",
      categorySlug: "tools",
      subcategory: "Multimeters",
      subcategorySlug: "multimeters",
      brand: "MASTECH",
      badge: "Popular",
      inStock: true,
      description: "Professional digital multimeter with auto-ranging and backlight.",
      specifications: JSON.stringify({
        "DC Voltage": "600V",
        "AC Voltage": "600V",
        "DC Current": "10A",
        "AC Current": "10A",
        Resistance: "60MΩ",
        Features: "Auto-ranging, Backlight, Data hold",
      }),
    },
    {
      name: "60W Soldering Iron Kit",
      slug: "60w-soldering-kit",
      price: 599,
      originalPrice: 799,
      rating: 4.6,
      reviews: 345,
      image: "/60w-soldering-iron-kit.jpg",
      category: "Tools & Equipment",
      categorySlug: "tools",
      subcategory: "Soldering Equipment",
      subcategorySlug: "soldering",
      brand: null,
      badge: null,
      inStock: true,
      description: "Complete soldering iron kit with stand, solder wire, and accessories.",
      specifications: JSON.stringify({
        Power: "60W",
        "Temperature Range": "200°C - 450°C",
        Includes: "Iron, Stand, Solder wire, Tips, Desoldering pump",
        "Tip Type": "Replaceable",
      }),
    },
  ]

  console.log(`🌱 Seeding ${products.length} products...`)

  for (const product of products) {
    await prisma.product.upsert({
      where: { slug: product.slug },
      update: product,
      create: product,
    })
  }

  console.log("✅ Database seeded successfully!")
}

main()
  .catch((e) => {
    console.error("❌ Error seeding database:", e)
    process.exit(1)
  })
  .finally(async () => {
    await prisma.$disconnect()
  })
